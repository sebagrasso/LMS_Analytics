...
df = load_pickle("files/prod_oil_field_todecline")
...
fig = make_subplots(rows=9, cols=1, subplot_titles=["Oil Rate", "Liquid Rate", "Declinatory Summary"], 
...
fig.add_trace(go.Scatter(x=df_well[df_well.inyeccion_start==1].fecha, y=df_well[df_well.inyeccion_start==1]["inyeccion"], mode="markers", name="Inyeccion",
...
fig.add_trace(go.Scatter(x=df_well[df_well.estado==1].fecha, y=df_well[df_well.estado==1]["estado"], mode="markers", name="Estados",
...
fig.add_trace(go.Scatter(x=df_well[df_well.estimulacion.isnull()==False].fecha, y=df_well[df_well.estimulacion.isnull()==False]["estimulacion"], mode="markers", name="Estimulaciones",
...
fig.add_trace(go.Scatter(x=df_well[df_well.inyeccion_capa.isnull()==False].fecha, y=df_well[df_well.inyeccion_capa.isnull()==False]["inyeccion_capa"], mode="markers", name="Inyeccion x Capa",
...
table_data = decline_df[decline_df.identificador==well][['tramo', 'identificador', 'evento', 'fecha_evento', 
                                                         # 'fecha_inicial', 
...
table = go.Table(
                header=dict(values=list(table_data.columns),
                            ...
                            align='left'),
                cells=dict(values=[table_data[col].round(4) for col in table_data.columns],
                           fill_color='lavender',
                           align='left'),
                name=f"Table - {well}")

fig.add_trace(table, row=7, col=1)
